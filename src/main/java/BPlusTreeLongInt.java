...
class BPlusTreeLongInt {
    ...
    private abstract class Node {
        ...
        abstract boolean insert(long key, int value);

        abstract int get(long key);

        abstract int addTo(long key, int value);
    ...
    private class InternalNode extends Node {
        ...
        @Override
        synchronized boolean insert(long key, int value) {
            int index = getChild(key);
            Node child = children[index];
            if (child.isOverflow()) {
    ...
    private class LeafNode extends Node {
        ...
        LeafNode() {
            ...
            this.values = new int[DEFAULT_BRANCHING_FACTOR];
            keyCount = 0;
        }
